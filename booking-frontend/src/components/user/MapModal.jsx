...
function MapModal({ isOpen, onClose, initialData, onConfirm }) {
  ...
  const applyPickedLocation = useCallback(
    async (lat, lon) => {
      ...
      if (address) {
        ...
        setAddressQuery(`${lat.toFixed(5)}, ${lon.toFixed(5)}`)
      }
    },
    [resolveAddress],
  )
      ...
      <div className="w-full max-w-4xl rounded-2xl bg-white shadow-xl">
        ...
        <div className="relative h-[420px] w-full overflow-hidden">
          ...
          <button
            ...
            className="absolute left-3 top-3 z-[500] inline-flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
            onClick={() => {
          ...
          >
            ...
            {picked?.lat != null && picked?.lng != null ? (
              <>
                ...
                <Circle center={[picked.lat, picked.lng]} radius={radius * 1000} pathOptions={{ color: '#2563eb' }} />
              </>
            ) : null}
          </MapContainer>
        </div>
